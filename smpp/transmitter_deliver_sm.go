...
func NewDeliverSM(sif *types.SmsInternalFormat) (pdu.Body, error) {
	message := pdu.NewDeliverSM()
	message.Header().Len = sif.HeaderCommandLength
	message.Header().Seq = sif.HeaderSequenceNumber

	if err := message.Fields().Set(pdufield.DataCoding, uint8(sif.DataCoding)); err != nil {
		return message, fmt.Errorf("[%s] %s set error: %w", "NewDeliverSM", "DataCoding", err)
	}

	if err := message.Fields().Set(pdufield.SourceAddrNPI, sif.SourceAddrNPI); err != nil {
		return message, fmt.Errorf("[%s] %s set error: %w", "NewDeliverSM", "SourceAddrNPI", err)
	}

	if err := message.Fields().Set(pdufield.SourceAddrTON, sif.SourceAddrTON); err != nil {
		return message, fmt.Errorf("[%s] %s set error: %w", "NewDeliverSM", "SourceAddrTON", err)
	}

	if err := message.Fields().Set(pdufield.SourceAddr, sif.Src); err != nil {
		return message, fmt.Errorf("[%s] %s set error: %w", "NewDeliverSM", "SourceAddr", err)
	}

	if err := message.Fields().Set(pdufield.DestAddrNPI, sif.DestAddrNPI); err != nil {
		return message, fmt.Errorf("[%s] %s set error: %w", "NewDeliverSM", "DestAddrNPI", err)
	}

	if err := message.Fields().Set(pdufield.DestAddrTON, sif.DestAddrTON); err != nil {
		return message, fmt.Errorf("[%s] %s set error: %w", "NewDeliverSM", "DestAddrTON", err)
	}

	if err := message.Fields().Set(pdufield.DestinationAddr, sif.Dst); err != nil {
		return message, fmt.Errorf("[%s] %s set error: %w", "NewDeliverSM", "DestinationAddr", err)
	}

	if err := message.Fields().Set(pdufield.ServiceType, sif.ServiceType); err != nil {
		return message, fmt.Errorf("[%s] %s set error: %w", "NewDeliverSM", "ServiceType", err)
	}

	if err := message.Fields().Set(pdufield.ESMClass, sif.ESMClass); err != nil {
		return message, fmt.Errorf("[%s] %s set error: %w", "NewDeliverSM", "ESMClass", err)
	}

	if err := message.Fields().Set(pdufield.PriorityFlag, sif.PriorityFlag); err != nil {
		return message, fmt.Errorf("[%s] %s set error: %w", "NewDeliverSM", "PriorityFlag", err)
	}

	if err := message.Fields().Set(pdufield.ProtocolID, sif.ProtocolID); err != nil {
		return message, fmt.Errorf("[%s] %s set error: %w", "NewDeliverSM", "ProtocolID", err)
	}

	if err := message.Fields().Set(pdufield.RegisteredDelivery, sif.RegisteredDelivery); err != nil {
		return message, fmt.Errorf("[%s] %s set error: %w", "NewDeliverSM", "RegisteredDelivery", err)
	}

	if err := message.Fields().Set(pdufield.ReplaceIfPresentFlag, sif.ReplaceIfPresentFlag); err != nil {
		return message, fmt.Errorf("[%s] %s set error: %w", "NewDeliverSM", "ReplaceIfPresentFlag", err)
	}

	if err := message.Fields().Set(pdufield.ScheduleDeliveryTime, sif.ScheduleDeliveryTime); err != nil {
		return message, fmt.Errorf("[%s] %s set error: %w", "NewDeliverSM", "ScheduleDeliveryTime", err)
	}

	if err := message.Fields().Set(pdufield.SMDefaultMsgID, sif.SMDefaultMsgID); err != nil {
		return message, fmt.Errorf("[%s] %s set error: %w", "NewDeliverSM", "SMDefaultMsgID", err)
	}

	if err := message.Fields().Set(pdufield.ValidityPeriod, nil); err != nil {
		return message, fmt.Errorf("[%s] %s set error: %w", "NewDeliverSM", "ValidityPeriod", err)
	}

	if err := message.Fields().Set(pdufield.SMLength, sif.SMLength); err != nil {
		return message, fmt.Errorf("[%s] %s set error: %w", "NewDeliverSM", "SMLength", err)
	}

	if sif.WhereIsText == types.SmsTextIntoText {
		switch pdutext.DataCoding(sif.DataCoding) {
		case pdutext.Latin1Type:
			if err := message.Fields().Set(pdufield.ShortMessage, pdutext.Latin1(sif.Text)); err != nil {
				return message, fmt.Errorf("[%s] %s with Latin1 set error: %w", "NewDeliverSM", "ShortMessage", err)
			}
		case pdutext.ISO88595Type:
			if err := message.Fields().Set(pdufield.ShortMessage, pdutext.ISO88595(sif.Text)); err != nil {
				return message, fmt.Errorf("[%s] %s with ISO88595 set error: %w", "NewDeliverSM", "ShortMessage", err)
			}
		case pdutext.UCS2Type:
			if err := message.Fields().Set(pdufield.ShortMessage, pdutext.UCS2(sif.Text)); err != nil {
				return message, fmt.Errorf("[%s] %s with UCS2 set error: %w", "NewDeliverSM", "ShortMessage", err)
			}
		default:
			if err := message.Fields().Set(pdufield.ShortMessage, pdutext.Raw(sif.Text)); err != nil {
...
func (t *Transmitter) SendDeliverSM(sm *ShortMessage, p pdu.Body) (*ShortMessage, error) {
	resp, err := t.do(p)
	if err != nil {
		return nil, err
	}
